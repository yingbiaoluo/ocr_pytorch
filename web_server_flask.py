...
with open('detect_text/config/Det_ResNet18_fpn.yaml') as f:
...
def load_model():
    ...
    model_recognition = RecModel(config_rec, ch=3, nc=nc, training=False).to(device)
...
def infer_detection(img):
    ...
    end_time = time.time()
    print('infer time of detection: {:.5f}s'.format(end_time - start_time))
...
def infer_recognition(image_batch):
    ...
    print('infer time of recognition: {:.5f}s'.format(time.time() - start_time))
...
def get_image():
    ...
    if request.method == 'POST':
        ...
        print('get data time: {:.5f}s'.format(time.time() - begin_time))
        result = handler(data)
    else:
        result = 'please input an image'
    print('total time: {:.5f}s'.format(time.time() - begin_time))
    return result

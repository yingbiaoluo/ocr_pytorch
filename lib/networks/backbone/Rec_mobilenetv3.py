...
from lib.networks.CommonModules import ConvBNACT, SEBlock
...
class MobileNetV3(nn.Module):
    ...
    def forward(self, x):  # [bs, 3, 32, 280]
        x = self.conv1(x)  # [bs, 8, 16, 140]
        x = self.block_list(x)  # [bs, 80, 2, 140]
        x = self.conv2(x)  # [bs, 480, 2, 140]
        x = self.pool(x)  # [bs, 480, 1, 70]
        return x
